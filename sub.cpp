...
{
    printf("Received topic: %s ", natsMsg_GetSubject(msg) );
    printf("length %d ", natsMsg_GetDataLength(msg) );
    unsigned char *dPtr = (unsigned char *)natsMsg_GetData(msg);
    for ( int ii = 0; ii < natsMsg_GetDataLength(msg); ii++ )
    {
       printf(" %3d", dPtr[ii] );
    }
    printf("\n");
